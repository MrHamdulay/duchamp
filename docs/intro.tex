...
on the sky -- often, but not necessarily, corresponding to Equatorial
or Galactic coordinates), while the third dimension, $z$, is the
...
has a unique value of flux or intensity or brightness (or something
...
\item The list of objects is condensed by merging neighbouring objects
  ...
  There are certain criteria the user can specify that objects must
  meet: minimum numbers of spatial pixels and spectral channels, and
  minimum separations between neighbouring objects. Those that do not
  meet these criteria are either deleted from the list, or merged with
  their nearby neighbours.
