...
set with the \texttt{minChannels}, \texttt{minPix} parameters and
\texttt{minVoxels}. The channel requirement means a source must have
at least one set of \texttt{minChannels} consecutive channels to be
accepted. The spatial pixels (\texttt{minPix}) requirement refers to
distinct spatial pixels (which are possibly in different channels),
...
\texttt{flagTwoStageMerging=false}.
