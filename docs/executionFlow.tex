...
spectrum is independently smoothed, and then put together to form the
smoothed cube. This is then treated in the same way as the
...
\texttt{hanningWidth}). The coefficients of the Hanning filter are
defined by
\[
\frac{1+\cos(\pi x/a)}{2},\ \frac{-(a+1)}{2}\leq x \leq \frac{a+1}{2},
\]
and zero elsewhere. Note that the width specified must be an odd
integer (if the parameter provided is even, it is incremented by one).
