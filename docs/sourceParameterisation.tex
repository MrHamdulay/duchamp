...
expression $S_{xx}-S_{yy} + 4 S_{xy}^2 < 0$, in which case the
calculation of $a$ and $b$ will fail. These situations are often where
the moment-0 map is very disordered with no clear primary axis. 
...
The algorithm will first try to calculate $a,b,\theta$ by weighting
